...
fig6 <- function(dat=comp, filename=NULL) {
  ...
  p6 <- ggplot(fig6df %>% filter(region != "Amygdala"),
    ...
    #facet_wrap(~regcomp, scale="free", ncol=3) +
    facet_wrap(~regcomp, scale="free", ncol=3,
               labeller = as_labeller(setNames(lbls,
                                               levels(fig6df$regcomp)))) +
...
sobel_test <- function(lmfit2, lmfit4) {
  #
  # calculate sobel test
  #
  alpha <- coef(lmfit2)[2]
  beta <- coef(lmfit4)[3]
  sigma1 <- (coef(summary(lmfit2)))[4]^2
  sigma2 <- (coef(summary(lmfit4)))[6]^2

  # sobel
  z1 <- (alpha * beta)/sqrt(beta^2 * sigma1 + alpha^2 * sigma2)
  pval1 <- 2 * pnorm(abs(z1), lower=FALSE)

  # aroian
  z2 <- (alpha * beta)/sqrt(beta^2 * sigma1 + alpha^2 * sigma2 +
                           sigma1*sigma2)
  pval2 <- 2 * pnorm(abs(z2), lower=FALSE)

  # goodman
  z3 <- (alpha * beta)/sqrt(beta^2 * sigma1 + alpha^2 * sigma2 -
                           sigma1*sigma2)
  pval3 <- 2 * pnorm(abs(z3), lower=FALSE)
  return(list(z1, z2, z3, pval1, pval2, pval3))

}


sobel_test_boot <- function(datb) {

  x <- datb$eta[datb$roi=="right_caudate_roi"&datb$component=="alpha"]
  m <- datb$beta[datb$roi=="right_caudate_roi"&datb$component=="alpha"]
  y <- datb$caps[datb$roi=="right_caudate_roi"&datb$component=="alpha"]
  data <- data.frame(x=x, m=m, y=y)
  tm <- test_mediation(data, "x", "y", "m", test="sobel",
                       method="regression")
  return(tm)
